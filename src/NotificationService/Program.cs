...
var host = Environment.GetEnvironmentVariable("NOTIFICATION_SERVICE_DB_HOST") 
    ?? Environment.GetEnvironmentVariable("RENDER_DB_HOST") 
    ?? "localhost";
    
var port = Environment.GetEnvironmentVariable("NOTIFICATION_SERVICE_DB_PORT") 
    ?? Environment.GetEnvironmentVariable("RENDER_DB_PORT") 
    ?? "5432";
    
var database = Environment.GetEnvironmentVariable("NOTIFICATION_SERVICE_DB_NAME") 
    ?? "cosre_notificationservice_local";
    
var username = Environment.GetEnvironmentVariable("NOTIFICATION_SERVICE_DB_USER") 
    ?? Environment.GetEnvironmentVariable("RENDER_DB_USER") 
    ?? "cosre_admin";
    
var password = Environment.GetEnvironmentVariable("NOTIFICATION_SERVICE_DB_PASSWORD") 
