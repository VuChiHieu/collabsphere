...
var host = Environment.GetEnvironmentVariable("EVALUATION_SERVICE_DB_HOST") 
    ?? Environment.GetEnvironmentVariable("RENDER_DB_HOST") 
    ?? "localhost";
    
var port = Environment.GetEnvironmentVariable("EVALUATION_SERVICE_DB_PORT") 
    ?? Environment.GetEnvironmentVariable("RENDER_DB_PORT") 
    ?? "5432";
    
var database = Environment.GetEnvironmentVariable("EVALUATION_SERVICE_DB_NAME") 
    ?? "cosre_evaluationservice_local";
    
var username = Environment.GetEnvironmentVariable("EVALUATION_SERVICE_DB_USER") 
    ?? Environment.GetEnvironmentVariable("RENDER_DB_USER") 
    ?? "cosre_admin";
    
var password = Environment.GetEnvironmentVariable("EVALUATION_SERVICE_DB_PASSWORD") 
